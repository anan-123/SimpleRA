...
class BufferManager{
    ...
    deque<Page> pages;
    deque<PageMatrix>matrixpages;
    bool inPool(string pageName);
    bool inMatrixPool(string pageName);
    Page getFromPool(string pageName);
    PageMatrix getFromMatrixPool(string pageName);
    Page insertIntoPool(string tableName, int pageIndex);
    PageMatrix insertIntoMatrixPool(string matrixName, int row_start, int col_start, int row_end, int col_end);
    public:
    
    BufferManager();
    Page getPage(string tableName, int pageIndex);
    PageMatrix getMatrixPage(string matrixName, int row_start, int col_start, int row_end, int col_end);
    void writePage(string pageName, vector<vector<int>> rows);
    void deleteFile(string tableName, int pageIndex);
    void deleteMatrixFile(string matrixName, int row_start, int col_start, int row_end, int col_end);
    void deleteFile(string fileName);
    void writePage(string tableName, int pageIndex, vector<vector<int>> rows, int rowCount);
    void writeMatrixPage(string matrixName, int row_start, int col_start, int row_end, int col_end, vector<vector<int>>& data);
};